...
int main()
{
	...
	Texture2D tex2("awesomeface.png", 1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(1, -0.6, 0));
		model = glm::rotate(model, (float)glfwGetTime() * glm::radians(100.0f), glm::vec3(0.5f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.4f, 0.4f, 0.4f));
		ourShader.setMat("model", model);

		// Camera
		ourShader.setMat("view", camera.view_matrix());
		ourShader.setMat("projection", camera.projection_matrix());
